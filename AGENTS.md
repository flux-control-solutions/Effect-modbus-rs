...
- **Retry policies** — `src/retry.ts` builds error-aware `Schedule`s (exponential + jitter, per-error curves, shared attempt budget). Nothing retries implicitly: a policy only applies where a caller pipes through `retryModbus` / `retryModbusWithReconnect`. Keep it that way — predictable default timing is a deliberate design decision (see issue #3). Jitter is on by default within a policy. These are application-level retries: they must not be combined with `modbus-rs`'s transport-level `retryAttempts`/`retryDelayMs`/`retryBackoffStrategy` transport options, since neither layer knows about the other.
